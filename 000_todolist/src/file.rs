...
impl FileSystem {
      ...
      if fields.len() < 4 {
        return Err(
          io::Error::new(
            ...
            format!("[Line {}] Not enough fields: expected 4, got {}", line_num + 1, fields.len()),
          )
        );
      }

      let key = fields[0].to_string();
      let timestamp = fields[1].parse::<u64>()
  ...
  pub fn add(&self, key: String, record: &ToDoListRecord) -> io::Result<()> {
    ...
    writeln!(
      ...
      key,
      SEPERATOR,
      record.timestamp.duration_since(UNIX_EPOCH).unwrap().as_secs().to_string(),
      SEPERATOR,
      record.content,
      SEPERATOR,
      record.status
  ...
  pub fn edit(&self, key: String, record: &ToDoListRecord) -> io::Result<()> {
    ...
    let file_output = File::create("./data/db.tmp")?;
    ...
    fs::rename("./data/db.tmp", &self.path)?;
