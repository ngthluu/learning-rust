...
impl ToDoList {
  pub fn load() -> Result<ToDoList, String> {
    let file_system = FileSystem {
      path: "./data/db.csv".into(),
    };

    file_system.load_data()
      .map(|lst| ToDoList {
        file_system,
        lst
      })
  ...
  pub fn add(&mut self, content: String) -> Result<String, String> {
    ...
    self.file_system.add(key.clone(), &record)
      .map_err(|e| e.to_string())?;
  ...
  pub fn edit(&mut self, key: String, content: String) -> Result<(), String> {
    if let Some(val) = self.lst.get_mut(&key) {
      val.content = content;
      self.file_system.edit(key, val)
        .map_err(|e| e.to_string())?;
      Ok(())
    } else {
      Err(format!("[Edit {} failed] Key is not exist", key))
    }
  }
  ...
  pub fn check(&mut self, key: String) -> Result<(), String>{
    if let Some(val) = self.lst.get_mut(&key) {
      ...
      Err(format!("[Check {} failed] Key is not exist", key))
    }
  }
  ...
  pub fn delete(&mut self, key: String) -> Result<(), String>{
    if let Some(val) = self.lst.get_mut(&key) {
      ...
      Err(format!("[Delete {} failed] Key is not exist", key))
    }
  }
}
